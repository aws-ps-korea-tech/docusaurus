...
## Bacterial Genome data 분석 {#bkmrk-bacterial-genome-dat}

이제 몇 가지 샘플 코드를 실행해 보겠습니다.

**[bwa](https://bio-bwa.sourceforge.net/){target="_blank" rel="noopener"}, [samtools](http://www.htslib.org/){target="_blank" rel="noopener"} 도구가 설치되어 있는지 확인해보겠습니다.\**
...
``` {#bkmrk-mkdir--p-%2Ftmp%2Foutbre}
mkdir -p /tmp/outbreaks/SG-M1
```

``` {#bkmrk-cd-%2Ftmp%2Foutbreaks%2Fsg}
cd /tmp/outbreaks/SG-M1
```

``` {#bkmrk-wget-ftp%3A%2F%2Fftp.ncbi.}
...
``` {#bkmrk-gunzip-gcf_001275545}
gunzip GCF_001275545.2_ASM127554v2_genomic.fna.gz
```

``` {#bkmrk-mv-gcf_001275545.2_a}
mv GCF_001275545.2_ASM127554v2_genomic.fna SG-M1.fna
```

2\. Map and call SNPs:\
**Note:** For an annotation of the programs used below and other bioinformatics tools, check out our course [github page](https://github.com/swainechen/chenlab-training){.highlight}.

**Reference indexing**

``` {#bkmrk-bwa-index-sg-m1.fna}
bwa index SG-M1.fna
```

**Mapping**\

``` {#bkmrk-bwa-mem-sg-m1.fna-%2Ft}
